...
# --- MPS compatibility change: Import torch ---
import torch
...
def main():
    ...
    # --- MPS compatibility change: Check for MPS device and set it ---
    device = "cpu"
    ...
    # --- MPS compatibility change: Move YOLO model to the selected device ---
    yolo        = YOLO(opt.weights).to(device)
    ...
    while True:
        ...
        if len(buf["parts"]) == buf["total"]:
            ...
            if rf_id % RF_INT == 0:
                # The YOLO model automatically handles moving the frame to the MPS device
                yres = yolo(frame, conf=opt.conf, max_det=1, verbose=False)[0]
                # --- MPS compatibility change: Move results to CPU for supervision ---
                dets = sv.Detections.from_ultralytics(yres.cpu())
                frame = box_annot.annotate(frame.copy(), dets)
