...
try:
    ...
    from external_controller import process_real_camera_frame
...
class CombinedVisualizer:
    ...
    def __init__(self, data_receiver):
        ...
        self.status_area = pygame.Rect(740, 0, 540, 240)  # Moved right by 100px, reduced width by 100px
        self.control_area = pygame.Rect(0, 480, 900, 280)  # Keep as is
        # Move 3D plot area to the right and make it narrower
        self.plot_3d_area = pygame.Rect(740, 240, 540, 540)  # Moved right by 100px, reduced width by 100px
        ...
        self.input_values = {
            ...
            "sim_kp": 0.1, "sim_ki": 0.0,
            "sim_kd": 0.0, "real_kp": 10550.0,
            "real_ki": 0.0, "real_kd": 0.0,
            "sim_surge_mult": 1.0, "sim_strafe_mult": 1.0,
            "sim_yaw_mult": 1.0, "sim_heave_mult": 1.0,
            "real_surge_mult": 1.0, "real_strafe_mult": 1.0,
            "real_yaw_mult": 1.0, "real_heave_mult": 1.0,
        }
    ...
    def draw_camera_panel(self):
        ...
        # Get the appropriate frame based on toggle
        frame = None
        if self.show_sim_camera:
            with self.data_receiver.sim_frame_lock:
                frame = self.data_receiver.latest_sim_frame
                if frame is not None:
                    frame = frame.copy()  # Copy to avoid lock contention
                    # Process sim frame if external controller available
                    if hasattr(self, '_external_controller_ref') and self._external_controller_ref:
                        frame = self._external_controller_ref.process_sim_camera_frame(frame)
        else:
            with self.data_receiver.frame_lock:
                ...
                if frame is not None:
                    frame = frame.copy()  # Copy to avoid lock contention
                    # Process real frame
                    frame = process_real_camera_frame(frame)
    ...
    def draw_control_panel(self):
        ...
        sim_color = "#90EE90"
        self.screen.blit(self.font_large.render("SIMULATOR PID", 1, sim_color), (x, y + 80))
        self.screen.blit(self.font_bold.render("Kp", 1, sim_color), (self.sim_kp_box.x, self.sim_kp_box.y + label_y_offset))
        self.screen.blit(self.font_bold.render("Ki", 1, sim_color), (self.sim_ki_box.x, self.sim_ki_box.y + label_y_offset))
        self.screen.blit(self.font_bold.render("Kd", 1, sim_color), (self.sim_kd_box.x, self.sim_kd_box.y + label_y_offset))
        
        # Vertical separator
        sep_x = self.sim_kd_box.right + 20
        pygame.draw.line(self.screen, (100, 100, 100), (sep_x, y + 60), (sep_x, y + 240), 2)
        
        # Sim multiplier labels (green)
        self.screen.blit(self.font_bold.render("Surge", 1, sim_color), (self.sim_surge_box.x, self.sim_surge_box.y + label_y_offset))
        self.screen.blit(self.font_bold.render("Strafe", 1, sim_color), (self.sim_strafe_box.x, self.sim_strafe_box.y + label_y_offset))
        self.screen.blit(self.font_bold.render("Yaw", 1, sim_color), (self.sim_yaw_box.x, self.sim_yaw_box.y + label_y_offset))
        self.screen.blit(self.font_bold.render("Heave", 1, sim_color), (self.sim_heave_box.x, self.sim_heave_box.y + label_y_offset))
        
        # Pink color for real sub labels
        real_color = "#FFB6C1"
        self.screen.blit(self.font_large.render("REAL SUB PID", 1, real_color), (x, y + 160))
        self.screen.blit(self.font_bold.render("Kp", 1, real_color), (self.real_kp_box.x, self.real_kp_box.y + label_y_offset))
        self.screen.blit(self.font_bold.render("Ki", 1, real_color), (self.real_ki_box.x, self.real_ki_box.y + label_y_offset))
        self.screen.blit(self.font_bold.render("Kd", 1, real_color), (self.real_kd_box.x, self.real_kd_box.y + label_y_offset))
        
        # Same for real sub labels
        self.screen.blit(self.font_bold.render("Surge", 1, real_color), (self.real_surge_box.x, self.real_surge_box.y + label_y_offset))
        self.screen.blit(self.font_bold.render("Strafe", 1, real_color), (self.real_strafe_box.x, self.real_strafe_box.y + label_y_offset))
        self.screen.blit(self.font_bold.render("Yaw", 1, real_color), (self.real_yaw_box.x, self.real_yaw_box.y + label_y_offset))
        self.screen.blit(self.font_bold.render("Heave", 1, real_color), (self.real_heave_box.x, self.real_heave_box.y + label_y_offset))
        ...
        input_configs = [
            ...
            (self.sim_surge_box, self.sim_surge_text, "sim_surge_mult"), (self.sim_strafe_box, self.sim_strafe_text, "sim_strafe_mult"),
            (self.sim_yaw_box, self.sim_yaw_text, "sim_yaw_mult"), (self.sim_heave_box, self.sim_heave_text, "sim_heave_mult"),
            ...
            (self.real_surge_box, self.real_surge_text, "real_surge_mult"), (self.real_strafe_box, self.real_strafe_text, "real_strafe_mult"),
            (self.real_yaw_box, self.real_yaw_text, "real_yaw_mult"), (self.real_heave_box, self.real_heave_text, "real_heave_mult"),
        ]
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.camera_toggle_checkbox.collidepoint(event.pos):
                    ...
                    return {}  # Return early - remove the unreachable clicked_on_ui = True line
                ...
                box_map = {
                    "steps": self.steps_box, "delay": self.delay_box, 
                    "sim_kp": self.sim_kp_box, "sim_ki": self.sim_ki_box, "sim_kd": self.sim_kd_box,
                    "sim_surge_mult": self.sim_surge_box, "sim_strafe_mult": self.sim_strafe_box, 
                    "sim_yaw_mult": self.sim_yaw_box, "sim_heave_mult": self.sim_heave_box,
                    "real_kp": self.real_kp_box, "real_ki": self.real_ki_box, "real_kd": self.real_kd_box,
                    "real_surge_mult": self.real_surge_box, "real_strafe_mult": self.real_strafe_box,
                    "real_yaw_mult": self.real_yaw_box, "real_heave_mult": self.real_heave_box
                }
    ...
    async def run_controller_async(self, num_steps, fwcmd, runtime):
        ...
        try:
            while not shutdown:
                ...
                # Apply sim multipliers
                sim_surge = surge * self.visualizer.input_values["sim_surge_mult"]
                sim_strafe = strafe * self.visualizer.input_values["sim_strafe_mult"]
                sim_heave = heave * self.visualizer.input_values["sim_heave_mult"]
                sim_yaw_cmd = yaw_cmd * self.visualizer.input_values["sim_yaw_mult"]

                # Apply real multipliers
                real_surge = surge * self.visualizer.input_values["real_surge_mult"]
                real_strafe = strafe * self.visualizer.input_values["real_strafe_mult"]
                real_heave = heave * self.visualizer.input_values["real_heave_mult"]
                real_yaw_cmd = yaw_cmd * self.visualizer.input_values["real_yaw_mult"]
                ...
                real_f = self.calculate_thruster_forces(
                    real_surge, real_strafe, real_heave, real_yaw_cmd, "real", real_stab_f
                )
                ...
                try:  # SYNC/ASYNC
                    if self.sync_mode == 1 and loop_count % 50 == 0:  # Only print every 50 loops
                        print(f" SYNC: Waiting for {num_steps} steps to complete...")
                        # No timeout - let it take as long as needed
                        await asyncio.gather(*tasks, return_exceptions=True)
                    else:
                        if loop_count % 50 == 0:  # Only print every 50 loops  
