...
class CombinedVisualizer:
    ...
    def __init__(self, data_receiver):
        ...
        self.pyramid_collection = Poly3DCollection([], facecolors=self.pyramid_face_colors, linewidths=0.8, edgecolors='k', alpha=0.6)
        self.setup_3d_plot()
        self.rotation_fix = np.eye(3)
        self.hud_data = {'armed': False, 'light_on': False, 'stabilization': True, 'yaw_kp': 0.0, 'trans_scale': 0.8}
    ...
    def setup_3d_plot(self):
        ...
        self.frame_lines = [self.ax_3d.plot([], [], [], c=c, lw=3, label=l)[0] for c, l in zip(['red', 'green', 'blue'], ['X (Roll)', 'Y (Pitch)', 'Z (Yaw)'])]
        self.ax_3d.legend()
    ...
    def update(self, imu_override: Optional[np.ndarray] = None):
        ...
        # HUD
        self.screen.blit(self.font.render(f"ARMED: {self.hud_data['armed']}", 1, (255, 0, 0) if self.hud_data['armed'] else (0, 255, 0)), (10, 10))
        self.screen.blit(self.font.render(f"LIGHT: {'ON' if self.hud_data['light_on'] else 'OFF'}", 1, (255, 255, 0)), (10, 30))
        self.screen.blit(self.font.render(f"STABILIZE: {'ON' if self.hud_data.get('stabilization', False) else 'OFF'}", 1, (0, 255, 255)), (10, 50))
        self.screen.blit(self.font.render(f"Yaw Kp: {self.hud_data['yaw_kp']:.2f}", 1, (255, 255, 255)), (10, 70))
        self.screen.blit(self.font.render(f"Move Scale: {self.hud_data.get('trans_scale', 0.8):.2f}", 1, (255, 255, 255)), (10, 90))

        imu_data = self.data_receiver.latest_imu_data
        if imu_data is None and imu_override is not None:
            w, x, y, z = imu_override
            r = np.arctan2(2*(w*x+y*z), 1-2*(x**2+y**2)); p = np.arcsin(np.clip(2*(w*y-z*x), -1, 1)); y_angle = np.arctan2(2*(w*z+x*y), 1-2*(y**2+z**2))
            imu_data = {'quaternion': imu_override, 'euler_deg': {'roll': np.degrees(r), 'pitch': np.degrees(p), 'yaw': np.degrees(y_angle)}}

        if imu_data and 'euler_deg' in imu_data:
            pygame.draw.line(self.screen, (100, 100, 100), (10, 115), (200, 115), 1)
            self.screen.blit(self.font.render(f"Roll:  {imu_data['euler_deg']['roll']:6.1f}°", 1, (255, 255, 255)), (10, 125))
            self.screen.blit(self.font.render(f"Pitch: {imu_data['euler_deg']['pitch']:6.1f}°", 1, (255, 255, 255)), (10, 145))
            self.screen.blit(self.font.render(f"Yaw:   {imu_data['euler_deg']['yaw']:6.1f}°", 1, (255, 255, 255)), (10, 165))
        ...
        if imu_data and 'quaternion' in imu_data and not np.any(np.isnan(imu_data['quaternion'])):
            ...
            self.pyramid_collection.set_verts([]); self.ax_3d.set_title('3D Orientation (Waiting...)')
...
class AsyncAUV:
    def __init__(self, server_ip="127.0.0.1", server_port=60001):
        ...
        self.connection_lock = asyncio.Lock(); self.connected = False
    ...
    async def _ensure_transport(self):
        async with self.connection_lock:
            ...
            try:
                ...
                self.transport, _ = await loop.create_datagram_endpoint(lambda: self.protocol, remote_addr=(self.server_ip, self.server_port))
                self.connected = True
    ...
    async def _send_command(self, data: bytes) -> Optional[bytes]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"AsyncAUV: Unexpected error: {e}"); self.connected = False
    ...
    async def reset(self) -> bool:
        pose = np.array([-10.71, -0.31, 0, 1, 0, 0, 0]); vel = np.zeros(3); ang_vel = np.zeros(3)
        data = struct.pack('<15f', float(MsgHeader.RESET.value), 0.0, *pose, *vel, *ang_vel)
        response = await self._send_command(data)
        return response is not None and response[0] == 0x00 and len(response) >= ACK
...
class RealSubmarineClient:
    def __init__(self, server_ip, control_port=10000):
        ...
        self.NEUTRAL_PULSE=1500; self.LIGHT_OFF=1100; self.LIGHT_ON=1900; self.AMP=300; self.LOOP_DURATION=0.02
    def _to_pwm(self, nf): return int(self.NEUTRAL_PULSE + nf * self.AMP)
    def _send_packet(self, cmd_dict):
        try:
            self.control_socket.sendto((json.dumps(cmd_dict) + '\n').encode('utf-8'), self.server_address)
        except Exception as e:
    ...
    def send_control(self, armed, light_on, forces):
        ...
        cmd = {'command': 'control', 'thruster_pulses': [self._to_pwm(v) for v in np.clip(forces / 100.0, -1.0, 1.0)],
               'light_pulse': self.LIGHT_ON if light_on else self.LIGHT_OFF, 'duration': self.LOOP_DURATION}
    ...
    def shutdown(self):
        ...
        self.send_control(False, False, np.zeros(6)); time.sleep(0.1); self.control_socket.close()
...
class BlueROVController:
    def __init__(self, auv_ip, auv_port, sub_ip):
        ...
        if pygame.joystick.get_count() == 0: raise RuntimeError("No gamepad connected.")
        ...
        self.manual_force_scale=100.0; self.translation_scale=0.8; self.rotation_scale=0.4
        self.pid_gains = { 'roll': PIDGains(0,0,0), 'pitch': PIDGains(0,0,0), 'yaw': PIDGains(0,0,0) }
        self.pid_states = { 'roll': PIDState(), 'pitch': PIDState(), 'yaw': PIDState() }
        self.target_attitude = np.array([1.,0.,0.,0.])
        self.kp_step=0.1; self.translation_scale_step=0.02; self.translation_scale_min, self.translation_scale_max = 0.05, 0.8
        self.LEFT_STICK_X=0; self.LEFT_STICK_Y=1; self.RIGHT_STICK_X=2; self.LEFT_TRIGGER=4; self.RIGHT_TRIGGER=5
        self.BTN_A=0; self.BTN_B=1; self.BTN_X=2; self.BTN_Y=3; self.BTN_MENU=6; self.BTN_LEFT_STICK=7
        self.BTN_LB=9; self.BTN_RB=10; self.BTN_HAT_UP=11; self.BTN_HAT_DOWN=12; self.BTN_HAT_LEFT=13; self.BTN_HAT_RIGHT=14
        self.BTN_MIDDLE=15; self.DEADZONE=0.1; self.dt=0.02; self.tuning_cooldown=0.2; self.last_tuning_time=0.0
        self.loop_times=[]; self.network_times=[]

    def apply_deadzone(self, v: float) -> float: return 0. if abs(v)<self.DEADZONE else np.sign(v)*(abs(v)-self.DEADZONE)/(1.-self.DEADZONE)
    ...
    def euler_to_quaternion(self, r: float, p: float, y: float) -> np.ndarray:
        cr,cp,cy=np.cos(r*0.5),np.cos(p*0.5),np.cos(y*0.5); sr,sp,sy=np.sin(r*0.5),np.sin(p*0.5),np.sin(y*0.5)
    ...
    def pid_control(self, axis: str, error: float) -> float:
        g,s=self.pid_gains[axis],self.pid_states[axis]; s.integral=np.clip(s.integral+error*self.dt,-10.,10.); o=g.kp*error+g.ki*s.integral+g.kd*(error-s.prev_error)/self.dt; s.prev_error=error; return 0. if np.isnan(o) else o

    def handle_button_events_old(self, event):
        if event.type != pygame.JOYBUTTONDOWN: return
        if event.button == self.BTN_MENU: self.armed = not self.armed; print(f"\n[{'ARMED' if self.armed else 'DISARMED'}]")
        elif event.button == self.BTN_X: self.stabilization_enabled = not self.stabilization_enabled; print(f"\nStabilization: {'ON' if self.stabilization_enabled else 'OFF'}")
        elif event.button == self.BTN_B: self.light_on = not self.light_on; print(f"\nLIGHT: {'ON' if self.light_on else 'OFF'}")
        elif event.button == self.BTN_MIDDLE: raise KeyboardInterrupt("Emergency stop")
        elif event.button == self.BTN_LEFT_STICK:
            print("\nResetting Orientation..."); self.target_attitude = np.array([1.,0.,0.,0.])
            if self.real_sub_client: self.real_sub_client.send_reset_command()
            asyncio.create_task(self.auv.reset())
        elif event.button in [self.BTN_Y, self.BTN_A]:
            r,p,y=self.quaternion_to_euler(self.target_attitude); y+=np.radians(10 if event.button == self.BTN_Y else -10); self.target_attitude=self.euler_to_quaternion(r,p,y); print(f"\n🎯 Target Yaw: {np.degrees(y):.1f}°")

        
        if time.time() - self.last_tuning_time > self.tuning_cooldown:
            tuned = False
            if event.button==self.BTN_HAT_UP: self.translation_scale=min(self.translation_scale+self.translation_scale_step,self.translation_scale_max); tuned=True
            elif event.button==self.BTN_HAT_DOWN: self.translation_scale=max(self.translation_scale-self.translation_scale_step,self.translation_scale_min); tuned=True
            elif event.button==self.BTN_HAT_RIGHT: self.pid_gains['yaw'].kp+=self.kp_step; tuned=True
            elif event.button==self.BTN_HAT_LEFT: self.pid_gains['yaw'].kp=max(0.,self.pid_gains['yaw'].kp-self.kp_step); tuned=True
            if tuned: print(f"\n🔧 Tuned: Scale={self.translation_scale:.2f}, YawKp={self.pid_gains['yaw'].kp:.1f}"); self.last_tuning_time=time.time()
    ...
    def get_controller_input(self) -> Tuple[float, float, float, float]:
        ...
        surge = -self.apply_deadzone(self.controller.get_axis(self.LEFT_STICK_Y)) * self.translation_scale
        strafe = self.apply_deadzone(self.controller.get_axis(self.LEFT_STICK_X)) * self.translation_scale
        yaw_cmd = -self.apply_deadzone(self.controller.get_axis(self.RIGHT_STICK_X)) * self.rotation_scale
        heave = (self.controller.get_axis(self.RIGHT_TRIGGER)-self.controller.get_axis(self.LEFT_TRIGGER))*0.5*self.translation_scale
    ...
    def calculate_stabilization_forces(self, current_attitude: np.ndarray) -> np.ndarray:
        ...
        _,_,current_yaw = self.quaternion_to_euler(current_attitude); _,_,target_yaw = self.quaternion_to_euler(self.target_attitude)
        if np.isnan(current_yaw) or np.isnan(target_yaw): return np.zeros(6)
        yaw_error_deg=(np.degrees(target_yaw-current_yaw)+180)%360-180; yaw_correction=self.pid_control('yaw',yaw_error_deg)
        stab_forces=np.array([yaw_correction, -yaw_correction, -yaw_correction, yaw_correction, 0, 0])
        return np.clip(stab_forces*0.1, -15., 15.)

    def calculate_thruster_forces(self, surge, strafe, heave, yaw_cmd, stab_forces) -> Tuple[np.ndarray, np.ndarray]:
        sim_f=np.array([surge-strafe+yaw_cmd, surge+strafe-yaw_cmd, -surge-strafe-yaw_cmd, -surge+strafe+yaw_cmd, heave, heave])
        real_f=np.array([surge-strafe+yaw_cmd, surge+strafe-yaw_cmd, surge+strafe+yaw_cmd, surge-strafe-yaw_cmd, heave, -heave])
        total_sim = sim_f*self.manual_force_scale + (stab_forces if self.stabilization_enabled else 0)
        total_real = real_f*self.manual_force_scale + (stab_forces if self.stabilization_enabled else 0)
        return np.clip(total_sim, self.minForce, self.maxForce), np.clip(total_real, self.minForce, self.maxForce)

    async def run_controller_async(self, num_steps, fwcmd, runtime):
        print("🚀 Starting ASYNC controller loop. Press MENU to arm."); self.visualizer = CombinedVisualizer(self.data_receiver_thread); self.data_receiver_thread.start()
        shutdown=False; loop_count=0; cmdstarttime=time.time()
        try:
            while not shutdown:
                loop_start=time.time();
                if time.time()-cmdstarttime > runtime+0.1: shutdown=True
                ...
                current_attitude = np.array(real_data['quaternion']) if real_data and 'quaternion' in real_data else None
                if current_attitude is None:
                    try:
                        sim_data = await asyncio.wait_for(self.auv.get_sensor_data(), timeout=0.005)
                        if sim_data and 'imu_quaternion' in sim_data: current_attitude = sim_data['imu_quaternion']
                    except asyncio.TimeoutError: pass
                if current_attitude is None: await asyncio.sleep(0.001); continue

                stab_f=self.calculate_stabilization_forces(current_attitude); sim_f,real_f=self.calculate_thruster_forces(surge,strafe,heave,yaw_cmd,stab_f)
                
                tasks = [self.auv.apply_ctrl(sim_f if self.armed else np.zeros(6), num_steps)]
                if self.real_sub_client:
                    loop = asyncio.get_event_loop()
                    tasks.append(loop.run_in_executor(None, self.real_sub_client.send_control, self.armed, self.light_on, real_f))
                
                network_start = time.time()
                try: await asyncio.wait_for(asyncio.gather(*tasks, return_exceptions=True), timeout=0.015)
                except asyncio.TimeoutError: print("Warning: Network commands timed out")
                self.network_times.append(time.time() - network_start)

                self.visualizer.hud_data.update({'armed':self.armed, 'light_on':self.light_on, 'stabilization':self.stabilization_enabled, 'yaw_kp':self.pid_gains['yaw'].kp, 'trans_scale':self.translation_scale})
                self.visualizer.update(imu_override=current_attitude)
                
                loop_time = time.time()-loop_start; self.loop_times.append(loop_time)
                if loop_count%100==0 and self.loop_times: print(f"Perf: Loop={np.mean(self.loop_times[-100:])*1000:.1f}ms, Net={np.mean(self.network_times[-100:])*1000:.1f}ms")
                loop_count+=1
                await asyncio.sleep(max(0, self.dt-loop_time))
        finally:
            print("Disarming and stopping..."); self.data_receiver_thread.stop(); self.data_receiver_thread.join(timeout=1); plt.close('all')
            try: await self.auv.apply_ctrl(np.zeros(6),1); await self.auv.close()
            except Exception as e: print(f"Could not cleanly close AUV connection: {e}")
            if self.real_sub_client: self.real_sub_client.shutdown()
            pygame.quit()
            if self.loop_times: print(f"Final Perf - Loop: {np.mean(self.loop_times)*1000:.1f}ms, Net: {np.mean(self.network_times)*1000:.1f}ms")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Async BlueROV2 Controller with High Performance Networking")
    parser.add_argument("--ip",type=str,default="127.0.0.1",help="IP of the MuJoCo/Unity simulator.")
    parser.add_argument("--port",type=int,default=60001,help="Port of the MuJoCo/Unity server.")
    ...
    parser.add_argument("--fwcmd",type=float,default=2.0,help="Time duration to apply forward force.")
    parser.add_argument("--runtime",type=float,default=5.0,help="Time duration to send commands.")
