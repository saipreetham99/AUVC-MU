...
class CombinedVisualizer:
    ...
    def update(self, imu_override: Optional[np.ndarray] = None):
        ...
        self.screen.blit(
            self.font.render(f"LIGHT: {light_text}", 1, (255, 255, 0)), (10, 30)
        )

        stab_text = "ON" if self.hud_data.get("stabilization", False) else "OFF"
        self.screen.blit(
            self.font.render(f"STABILIZE: {stab_text}", 1, (0, 255, 255)), (10, 50)
        )
        ...
        self.screen.blit(
            self.font.render(f"Real Kp: {real_kp:.2f}", 1, (255, 255, 255)), (10, 90)
        )
        ...
        if imu_data and "euler_deg" in imu_data:
            ...
            self.screen.blit(
                self.font.render(pitch_text, 1, (255, 255, 255)), (10, 165)
            )
...
class AsyncAUV:
        ...
        async with self.connection_lock:
            ...
            try:
                ...
                self.transport, _ = await loop.create_datagram_endpoint(
                    lambda: self.protocol, remote_addr=remote_addr
                )
    ...
    async def apply_ctrl(
        self, forces: np.ndarray, num_steps: int = 1
    ) -> Optional[dict]:
        if len(forces) != 6:
            raise ValueError("Forces array must have 6 elements")

        data = struct.pack(
            "<8f", float(MsgHeader.APPLY_CTRL.value), float(num_steps), *forces
        )
        response = await self._send_command(data)
    ...
    async def reset(self) -> bool:
        ...
        data = struct.pack(
            "<15f", float(MsgHeader.RESET.value), 0.0, *pose, *vel, *ang_vel
        )
        response = await self._send_command(data)
...
class BlueROVController:
    ...
    def pid_control(self, axis: str, error: float) -> float:
        ...
        # Calculate output
        output = (
            g.kp * error + g.ki * s.integral + g.kd * (error - s.prev_error) / self.dt
        )
    ...
    def get_controller_input(self) -> Tuple[float, float, float, float]:
        ...
        # Heave from triggers
        trigger_diff = self.controller.get_axis(
            self.RIGHT_TRIGGER
    ...
    async def run_controller_async(self, num_steps, fwcmd, runtime):
        ...
        try:
            while not shutdown:
                ...
                for e in pygame.event.get():
                    ...
                    if e.type == pygame.JOYBUTTONDOWN:
                        if e.button == self.BTN_MENU:
                            ...
                            self.target_attitude = self.euler_to_quaternion(
                                r, p, new_y_rad
                            )
                ...
                real_stab_f = (
                    self.calculate_stabilization_forces(current_attitude_real, "real")
                    * self.new
                )
                ...
                tasks = [
                    self.auv.apply_ctrl(sim_f if self.armed else np.zeros(6), num_steps)
                ]
                ...
                try:
                    await asyncio.wait_for(
                        asyncio.gather(*tasks, return_exceptions=True), timeout=0.015
                    )
                except asyncio.TimeoutError:
                ...
                self.visualizer.hud_data.update(
                    {
                        ...
                        "yaw_kp": self.pid_gains[
                            "yaw_sim"
                        ].kp,  # Show sim Kp in main display
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=60001,
        help="Simulation Port of the MuJoCo/Unity server.",
    )
    ...
    try:
        controller = BlueROVController(args.sim_ip, args.sim_port, args.sub_ip)
