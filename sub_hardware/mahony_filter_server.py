...
def _get_q_from_acc(acc):
    ...
    cy, sy = np.cos(0), np.sin(0); cp, sp = np.cos(pitch*0.5), np.sin(pitch*0.5); cr, sr = np.cos(roll*0.5), np.sin(roll*0.5)
...
def quaternion_to_euler(q: np.ndarray) -> Tuple[float, float, float]:
    ...
    sinp = 2*(q0*q2-q3*q1); pitch = np.arcsin(np.clip(sinp, -1, 1))
...
class MagnetometerCalibrator:
    ...
    def _fit_ellipsoid(self):
        ...
        with self.data_lock:
            ...
            D = np.c_[points**2, 2*points[:,0]*points[:,1], 2*points[:,0]*points[:,2], 2*points[:,1]*points[:,2], 2*points]
            v = np.ones(D.shape[0])
...
class AttitudeFilter:
    def __init__(self, frequency=100.0, kp=0.5, ki=0.1):
        self.q = np.array([1.0, 0.0, 0.0, 0.0]); self.bias = np.zeros(3)
        self.kp = kp; self.ki = ki; self.dt = 1.0 / frequency
...
class AttitudeServer:
    def __init__(self, host='0.0.0.0', port=9999, kp=0.5, ki=0.1, recalibrate=False):
        ...
        self.reference_quaternion = np.array([1.0, 0.0, 0.0, 0.0])  # Identity quaternion
        self.reference_lock = threading.Lock()

        self.running = False; self.clients = []; self.client_lock = threading.Lock()
        self.latest_icm = None; self.latest_mmc = None; self.data_lock = threading.Lock()
            ...
            try:
                ...
                while '\n' in buffer:
                    ...
                    if line == "RESET" or line == "TARE" or line == "ZERO":
                        ...
                        # Send acknowledgment
                        try:
                            ack_msg = json.dumps({"status": "RESET_OK", "timestamp": time.time()}).encode() + b'\n'
    ...
    def start_server(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1); s.bind((self.host, self.port))
            s.listen(5); s.settimeout(1.0); print(f"Attitude Server listening on {self.host}:{self.port}")
            print("Clients can send 'RESET' command to zero the orientation")
            
            while self.running:
                try:
                    c, addr = s.accept(); print(f"\nClient connected from {addr}")
                    with self.client_lock: self.clients.append(c)
                    
                    # Start command handler thread for this client
                    cmd_thread = threading.Thread(target=self.handle_client_commands, args=(c,), daemon=True)
                    cmd_thread.start()
    ...
    def broadcast_data(self, data):
        ...
        with self.client_lock:
            ...
            for c in self.clients:
                try: c.sendall(msg)
                except (ConnectionResetError, BrokenPipeError, OSError): disconnected.append(c)
            for c in list(set(disconnected)):
                ...
                self.clients.remove(c); c.close()

    def read_icm20602(self):
        try: from icm20602 import ICM20602; icm = ICM20602(); print("OK: ICM20602")
        except Exception as e: print(f"WARN: ICM20602: {e}. Using dummy data."); icm = None
        while self.running:
            try:
                if icm: d=icm.read_all(); ax,ay,az,gx,gy,gz=d.a.x,d.a.y,d.a.z,d.g.x,d.g.y,d.g.z
                else: ax,ay,az,gx,gy,gz=0,0,9.81,0.01,-0.02,0.03
                with self.data_lock: self.latest_icm={'ax':ax,'ay':ay,'az':az,'gx':np.radians(gx),'gy':np.radians(gy),'gz':np.radians(gz),'timestamp':time.time()}
                time.sleep(0.01)
            except Exception as e: print(f"\nICM20602 error: {e}"); time.sleep(1)

    def read_mmc5983(self):
        try: from mmc5983 import MMC5983; mmc = MMC5983(); print("OK: MMC5983")
        except Exception as e: print(f"WARN: MMC5983: {e}. Using dummy data."); mmc = None
        while self.running:
            try:
                if mmc:
                    d=mmc.read_data(); mx,my,mz=d.x,d.y,d.z
                else: mx,my,mz=25.,5.,-30.
                mag_raw = np.array([mx, my, mz])
                self.mag_calibrator.add_point(mag_raw) # Add raw point to calibrator
                with self.data_lock: self.latest_mmc={'mx':mx,'my':my,'mz':mz,'timestamp':time.time()}
                time.sleep(0.02)
            except Exception as e: print(f"\nMMC5983 error: {e}"); time.sleep(1)
    ...
    def _initialize_filter_state(self):
        ...
        if q0 is None:
            print("No valid initial orientation in cache. Place device level to initialize.")
            ...
            while time.time() < end_time:
                with self.data_lock:
                    if self.latest_icm:
                        acc_readings.append([self.latest_icm['ax'], self.latest_icm['ay'], self.latest_icm['az']])
                time.sleep(0.1)
            ...
            if not acc_readings:
                ...
                print(f"Initialized with new orientation from accelerometer. q0: {q0}")

            try:
                # Save the new q0 to cache
                if os.path.exists(CACHE_FILENAME):
                    with open(CACHE_FILENAME, 'r') as f: data = json.load(f)
                else: data = {}
                data['q0'] = q0.tolist()
                with open(CACHE_FILENAME, 'w') as f: json.dump(data, f, indent=4)
            except Exception as e:
    ...
    def filter_loop(self):
        print("Filter loop waiting for sensor data..."); time.sleep(1)
        ...
        while self.running:
            with self.data_lock: icm=self.latest_icm; mmc=self.latest_mmc
            if icm and mmc:
                ...
                self.broadcast_data({
                    ...
                    'euler': {'roll':r, 'pitch':p, 'yaw':y}  # Relative euler angles
                })
...
def main():
    parser = argparse.ArgumentParser(description='Attitude Server with Mahony filter, Online Calibration, and Tare functionality')
    parser.add_argument('--host', default='192.168.2.11', help='Server host IP')
    parser.add_argument('--port', type=int, default=9999, help='Server port')
    parser.add_argument('--kp', type=float, default=17.0, help='Proportional gain (Kp) for attitude correction. NOTE: Lower Kp is better for online mag calibration.')
    parser.add_argument('--ki', type=float, default=0.2, help='Integral gain (Ki) for gyro drift correction.')
    parser.add_argument('-c','--recalibrate', action='store_true', help='Force a new initial orientation reading, ignoring cached value.')
    args = parser.parse_args()

    # Note: Kp and Ki values are retuned for this new method.
    # The filter is now less reliant on the magnetometer for high-frequency correction.
    print(f"Starting server with Kp={args.kp}, Ki={args.ki}")
    server = AttitudeServer(host=args.host,port=args.port,kp=args.kp,ki=args.ki,recalibrate=args.recalibrate)
    server.run()
