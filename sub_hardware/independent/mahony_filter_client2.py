...
class SimpleAttitudeClient:
    ...
    def __init__(self, server_host='127.0.0.1', server_port=9999):
        ...
        self.ax_3d = None
        self.fig.suptitle(
            f'Attitude Orientation Tracking - {server_host}', fontsize=16)
    ...
    def setup_2d_plot(self):
        ...
        self.ax_euler.legend(loc='upper left')
        ...
        if self.ax_3d is None:
            ...
            for i in range(3):
                line, = self.ax_3d.plot(
                    [0, 0], [0, 0], [0, 0], c=colors[i], lw=4, label=labels[i], alpha=0.9)
                self.frame_lines.append(line)

            self.ax_3d.scatter([0], [0], [0], c='k', s=80)
    ...
    def process_data(self, data):
        if 'status' in data:
            if data['status'] == 'RESET_OK':
                print(f"Server confirmed orientation reset at {
    ...
    def animate(self, frame):
        if self.connected:
            self.status_text.set_text('Connected')
        ...
        if len(self.times) > 1:
            ...
            if times[-1] > self.time_window_size:
                self.ax_euler.set_xlim(
                    times[-1] - self.time_window_size, times[-1])
            else:
                self.ax_euler.set_xlim(0, self.time_window_size)
            ...
            if self.connected and self.ax_3d is not None and self.ax_3d.get_visible() and self.frame_lines:
                if self.latest_data and 'quaternion' in self.latest_data:
                    ...
                    self.frame_lines[0].set_data(
                        [0, x_axis_final[0]], [0, x_axis_final[1]])
                    ...
                    self.frame_lines[1].set_data(
                        [0, y_axis_final[0]], [0, y_axis_final[1]])
                    self.frame_lines[1].set_3d_properties([0, y_axis_final[2]])

                    # The Yaw line (blue, frame_lines[2]) is unchanged
                    self.frame_lines[2].set_data(
                        [0, z_axis_final[0]], [0, z_axis_final[1]])
                    self.frame_lines[2].set_3d_properties([0, z_axis_final[2]])

            rate = len(
                self.times) / ((times[-1] - times[0]) + 1e-6) if len(self.times) > 1 else 0.0
    ...
    def run(self):
        ...
        connect_thread = threading.Thread(
            target=self.try_initial_connection, daemon=True)
    ...
    def shutdown(self):
        ...
        # --- FIX: Check if the animation and its event source exist before stopping. ---
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Simple Attitude Visualization Client')
    ...
    try:
        creds = config[mode]
        server_host = creds['rov_ip']
        server_port = config.getint('DEFAULT', 'imu_and_depth_port')
