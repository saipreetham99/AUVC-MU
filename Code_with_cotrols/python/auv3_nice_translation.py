            ...
            if event.type == pygame.JOYBUTTONDOWN:
                ...
                # Real-time tuning controls
                    ...
                    if event.button == self.BTN_HAT_UP:
                        ...
                        # Increase stabilization force multiplier
                        old_mult = self.stabilization_force_multiplier
                        self.stabilization_force_multiplier = min(self.stabilization_force_multiplier + self.stabilization_mult_step, self.stabilization_mult_max)
                        if self.stabilization_force_multiplier != old_mult:
                            print(f"\n🔧 Stabilization Force Multiplier: {self.stabilization_force_multiplier:.2f} (↑)")
                            self.last_tuning_time = current_time
                    
                    elif event.button == self.BTN_Y:
                        # Decrease stabilization force multiplier
                        old_mult = self.stabilization_force_multiplier
                        self.stabilization_force_multiplier = max(self.stabilization_force_multiplier - self.stabilization_mult_step, self.stabilization_mult_min)
                        if self.stabilization_force_multiplier != old_mult:
                            print(f"\n🔧 Stabilization Force Multiplier: {self.stabilization_force_multiplier:.2f} (↓)")
                            self.last_tuning_time = current_time
